...
export default {
  ...
  methods: {
    ...
    dataSubmit() {
      if (this.form.id) {
        this.$api
          .put("web/type/document/" + this.form.id, this.form)
          .then((res) => {
            if (res.status == 200) {
              this.getData();
        ...
        this.$api.post("web/type/document", this.form).then((res) => {
          if (res.status == 201) {
            this.getData();
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$api.delete("web/type/document/" + id).then((res) => {
              if (res.status == 200) {
                this.getData();
    ...
    editData(id) {
      this.$api.get("web/type/document/" + id + "/edit").then((res) => {
        if (res.status == 200) {
          console.log(res);
          this.form = res.data;
        }
    ...
    getData() {
      this.$api.get("web/type/document").then((res) => {
        if (res.status == 200) {
          this.rows = res.data;
        }
      });
