...
export default {
  ...
  methods: {
    addData() {
      this.$bvModal.show("modal-neighborhood");
    },
    handleOk(bvModalEvt) {
    ...
    dataSubmit() {
      if (this.form.id) {
        this.$api.put("web/data/neighborhood/" + this.form.id, this.form).then((res) => {
          if (res.status == 200) {
            this.getData();
            toastr.success("Dato Actualizado");
            this.$bvModal.hide("modal-neighborhood");
          }
        });
      } else {
        this.$api.post("web/data/neighborhood", this.form).then((res) => {
          if (res.status == 201) {
            this.getData();
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$api.delete("web/data/neighborhood/" + id).then((res) => {
              if (res.status == 200) {
                this.getData();
    ...
    editData(id) {
      this.$api.get("web/data/neighborhood/" + id + "/edit").then((res) => {
        if (res.status == 200) {
          console.log(res);
          this.form = res.data;
        }
    ...
    getData() {
      this.$api.get("web/data/neighborhood").then((res) => {
        if (res.status == 200) {
          this.rows = res.data;
        }
      });
