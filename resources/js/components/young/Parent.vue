      ...
      <ValidationObserver ref="observer" v-slot="{ handleSubmit }">
        <form ref="form" @submit.prevent="handleSubmit(dataSubmit)">
          ...
          <div class="form-group">
            ...
            <ValidationProvider
              name="telefono"
              rules="required|min:7|max:15|regex:\d+"
              v-slot="{ errors }"
            >
              <input
                id="phone"
...
export default {
  ...
  methods: {
    addData() {
      this.$bvModal.show("modal-parent");
    },
    handleOk(bvModalEvt) {
    ...
    dataSubmit() {
      if (this.form.id) {
        this.$api
          .put("web/type/parent/" + this.form.id, this.form)
          .then((res) => {
            if (res.status == 200) {
              this.getData();
              toastr.success("Dato Actualizado");
              this.$bvModal.hide("modal-parent");
            }
          });
      } else {
        this.$api.post("web/type/parent", this.form).then((res) => {
          if (res.status == 201) {
            this.getData();
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$api.delete("web/type/parent/" + id).then((res) => {
              if (res.status == 200) {
                this.getData();
    ...
    editData(id) {
      this.$api.get("web/type/parent/" + id + "/edit").then((res) => {
        if (res.status == 200) {
          console.log(res);
          this.form = res.data;
        }
    ...
    getData() {
      this.$api.get("web/type/parent").then((res) => {
        if (res.status == 200) {
          this.rows = res.data;
        }
      });
