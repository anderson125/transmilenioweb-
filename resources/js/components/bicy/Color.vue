...
export default {
  ...
  methods: {
    ...
    dataSubmit() {
      if (this.form.id) {
        this.$api.put("web/data/color/" + this.form.id, this.form).then((res) => {
          if (res.status == 200) {
            this.getData();
            toastr.success("Dato Actualizado");
            this.$bvModal.hide("modal-color");
          }
        });
      } else {
        this.$api.post("web/data/color", this.form).then((res) => {
          if (res.status == 201) {
            this.getData();
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$api.delete("web/data/color/" + id).then((res) => {
              if (res.status == 200) {
                this.getData();
    ...
    editData(id) {
      this.$api.get("web/data/color/" + id + "/edit").then((res) => {
        if (res.status == 200) {
          console.log(res);
          this.form = res.data;
        }
    ...
    getData() {
      this.$api.get("web/data/color").then((res) => {
        if (res.status == 200) {
          this.rows = res.data;
        }
      });
