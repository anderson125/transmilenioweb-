...
export default {
  ...
  methods: {
    ...
    dataSubmit() {
      if (this.form.id) {
        this.$api.put("web/data/brand/" + this.form.id, this.form).then((res) => {
          if (res.status == 200) {
            this.getData();
        ...
        this.$api.post("web/data/brand", this.form).then((res) => {
          if (res.status == 201) {
            this.getData();
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$api.delete("web/data/brand/" + id).then((res) => {
              if (res.status == 200) {
                this.getData();
    ...
    editData(id) {
      this.$api.get("web/data/brand/" + id + "/edit").then((res) => {
        if (res.status == 200) {
          console.log(res);
          this.form = res.data;
        }
    ...
    getData() {
      this.$api.get("web/data/brand").then((res) => {
        if (res.status == 200) {
          this.rows = res.data;
        }
      });
