...
export default {
  ...
  methods: {
    ...
    dataSubmit() {
      if (this.form.id) {
        this.$api.put("web/status/bicy/" + this.form.id, this.form).then((res) => {
          if (res.status == 200) {
            this.getData();
        ...
        this.$api.post("web/status/bicy", this.form).then((res) => {
          if (res.status == 201) {
            this.getData();
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$api.delete("web/status/bicy/" + id).then((res) => {
              if (res.status == 200) {
                this.getData();
    ...
    editData(id) {
      this.$api.get("web/status/bicy/" + id + "/edit").then((res) => {
        if (res.status == 200) {
          console.log(res);
          this.form = res.data;
        }
    ...
    getData() {
      this.$api.get("web/status/bicy").then((res) => {
        if (res.status == 200) {
          this.rows = res.data;
        }
      });
