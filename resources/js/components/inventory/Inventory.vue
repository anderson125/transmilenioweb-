    ...
    async dataSubmit () {
      ...
      if (this.form.id) {
        ...
        if(!this.form.inputBicies.length){
          toastr.error('No ha seleccionado ninguna bicicleta'); return;
        }
        ...
        await this.$api.post("web/data/inventoryBicy", {inventories_id : this.form.id, bicies_code})
          .then((res) => { console.log(res); errors = res.data.response.errors; });
    ...
    editData(id) {
      ...
      this.$api.get("web/data/inventory/" + id).then((res) => {
        if (res.status == 200) {
          ...
          this.form = data ;
          this.biciesData = this.biciesRawData.filter(el => { let filtered = ! this.form.bicies.find(_el => _el.code == el.code); return !!filtered} );
